...
public class CsvAccTrasero {
    private File csvFile;

    // Constructor
    public CsvAccTrasero(Context context) {
    ...
    public void saveDataToCsv(double speedKmh, double latitude, double longitude,boolean ab,boolean accidentFlag ) {
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
            String currentTime = sdf.format(new Date()); // Obtiene la fecha y hora actual
            FileWriter writer = new FileWriter(csvFile, true);

            writer.append(String.format("\"%s\",\"%.2f\",\"%f\",\"%f\",\"%b\",\"%b\"\n",
                    currentTime, speedKmh, latitude, longitude, ab ,accidentFlag));
